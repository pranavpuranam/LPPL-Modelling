...
OUTPUT_CSV = "eurostoxx600_rolling_lppls_results.csv"
...
def run_rolling_lppls(
    ...
    for i, row in windows.iterrows():
        try:
            ...
            fit_row["window_dt_requested"] = row["dt"]
            ...
            print(
                ...
                f"t2={row['t2']} | dt={row['dt']} | "
                f"valid={fit_row['lppls_valid'].iloc[0]} | "
            ...
            print(f"{i + 1}/{len(windows)} failed | t2={row['t2']} | {e}")
...
def make_confidence_indicator(results):
    res = results.copy()
    ...
    confidence = (
        res.groupby("t2")
        .agg(
            total_fits=("lppls_valid", "size"),
            valid_fits=("lppls_valid", "sum"),
...
def plot_price_and_confidence(
    prices,
    confidence,
    smooth_window=3,
    title="STOXX Europe 600: Price and LPPLS Bubble Confidence",
):
    conf = confidence.copy()

    conf["confidence_smooth"] = (
    ...
    ax2.plot(
        ...
        conf["confidence_smooth"],
        linewidth=2.0,
        label="LPPLS bubble confidence",
    )

    ax2.set_ylabel("LPPLS bubble confidence")
    ax2.set_ylim(0, 1)
...
if __name__ == "__main__":
    ...
    confidence = make_confidence_indicator(results)

    confidence.to_csv("eurostoxx600_lppls_confidence.csv", index=False)
