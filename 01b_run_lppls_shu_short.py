# 01b: run short-horizon lppl fits
...
OUTPUT_CSV = "eurostoxx600_lppls_t2step4_shu_short_positive_fits.csv"  # raw fit output
CONFIDENCE_CSV = "eurostoxx600_lppls_t2step4_shu_short_positive_confidence.csv"  # confidence output
OUTPUT_PNG = "eurostoxx600_lppls_t2step4_shu_short_positive_price_confidence.png"  # plot output
DATE_COL = "Date"  # date column
Y_COL = "log_price"  # fitted series
M_BOUNDS = (0.1, 0.9)  # m bounds
OMEGA_BOUNDS = (6.0, 13.0)  # omega bounds
DT_MIN = 30  # shortest window
DT_MAX = 200  # longest window
DT_STEP = 10  # window step
T2_STEP = 4  # endpoint step
TC_MIN_DAYS = 1  # minimum tc after t2
TC_SEARCH_FRACTION = 1 / 3  # tc search limit
TC_VALID_FRACTION = 1 / 5  # tc validity limit
SEED = 42  # random seed
...
def fit_lppl_row(df, t1, t2, window_dt_requested):
    ...
    if len(window) < DT_MIN:  # check minimum observations
    ...
    window_calendar_days = max(t_last, 1.0)  # window length in days
    tc_lower = t_last + TC_MIN_DAYS  # lower tc bound
    tc_upper = t_last + TC_SEARCH_FRACTION * window_calendar_days  # upper tc search bound
    ...
    tc_days_after_t2 = int((tc_date - actual_t2).days)  # calculate tc offset
    damping = (m * abs(B)) / (omega * abs(C)) if abs(C) > 0 else np.inf  # calculate damping
    tc_search_upper_days_after_t2 = float(tc_upper - t_last)  # store search limit
    tc_valid_upper_days_after_t2 = float(TC_VALID_FRACTION * window_calendar_days)  # store valid limit
...
def run_lppls(df):
    windows = generate_rolling_lppl_windows(df)  # create rolling windows
    ...
    print("Rows in price file:", len(df))  # print price rows
    print("Expected LPPL fits:", len(windows))  # print fit count
    print("Windows per t2:", len(range(DT_MIN, DT_MAX + 1, DT_STEP)))  # print windows per endpoint
...
def plot_price_and_confidence(prices, confidence):
    ...
    ax2.plot(
        ...
        linestyle="--",
        linewidth=2.0,
        label="LPPL Shu-Short Positive Confidence",
...
if __name__ == "__main__":
    ...
    results = run_lppls(prices)  # run all lppl fits
    confidence = make_confidence(results)  # calculate confidence scores
    confidence.to_csv(CONFIDENCE_CSV, index=False)  # save confidence scores
    plot_price_and_confidence(prices, confidence)  # plot confidence scores
    print("Saved raw fits to:", OUTPUT_CSV)  # print raw save path
    print("Saved confidence scores to:", CONFIDENCE_CSV)  # print confidence save path
    print("Saved plot to:", OUTPUT_PNG)  # print plot save path
